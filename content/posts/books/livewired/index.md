...
- Our machinery (read brain) isn't fully preprogrammed, but instead shapes itself by interacting with the world.
- Why is the enemy of memory not time but other memories?
- Of all the objects our species has discovered on the planet, nothing rivals the complexity of our own brains.
...
- As the border wars rage throughout the lifetime of a brain, maps are redrawn so that the experiences and goals of a person are always reflected in the brain's structure.
- E.g. If an accountant switches her career to become a pianist, then the territory devoted to fingers grows and the territory devotes to numbers shrinks.
...
- Millions of years of evolution didn't presage the arrival of written language, much less a keyboard, and yet our brains have no trouble with these innovations.
- Experience changes the brain, and it retains the change.
- Neuroplasticity is a somewhat misleading term because it implies only molding once and never again, which isn't how brains are adaptable.
...
- The case of Matthew who had half of his brain removed due to seizures. He grew up normally and it's difficult to tell that he's missing half a brain.
- Unlike hardware, liveware endures.
- There is not an overarching framework for neuroplasticity/livewiring.
- This book lays out a framework/principles of livewiring.
...
- E.g. We're pre-equipped to absorb language.
- The fine structure of the brain reflects the environment to which it's exposed.
...
- The answer is that touch to the hand isn't only represented by the cells in the somatosensory cortex, but also by the cells they talk to downstream, and the cells they talk too.
...
- The same is true all over your body, patches that are neighbouring will tend to be more co-active than patches that aren't.
...
- By keeping the occipital cortex active during the night through dreaming.
...
- A key point is that these nighttime volley of activity are only directed at the occipital cortex through the lateral geniculate nucleus.
...
- Cochlear and retinal implants are evidence that the brain can handle data that it wasn't evolved for.
- But how?
...
- The brain is stunningly gifted at taking these signals and exacting patterns and meaning.
...
- Mother Nature can build new senses by building new peripherals.
...
- It all gets translated into spikes.
- We have evidence for this as some babies are born without a nose or eyes; no peripherals for a sense.
...
- E.g. Imagine feeding real-time patient data into a surgeons' back or the ISS health into an astronauts' back. You can replace staring at monitors with different patterns of touch, summarizing data and making it more intuitive and experiential.
...
- The answer I believe is that the pathway of the information determines the meaning of the signal.
...
- Language can only communicate shared experiences.
...
- The main principle: the motor areas optimize themselves to drive the available machinery.
...
- We nurture a feedback loop between the internal and external worlds.
...
- We use extensions of our body in jobs such as forklift operators and crane operators.
...
- If you can control it, it becomes a part of you.
- This explains the disorder of asomatognosia or the inability to know one's body.
...
- Another answer is one-trial learning.
- E.g. Emergency mechanisms such as learning to not touch a hot stove but also learning a new concept such as pomegranate.
- Your brain allocates more territory to a stimulus because of the presence of acetylcholine indicates that it must be important.
- It's a universal mechanism as it can broadcast across the entire brain.
...
- The fundamental fact that brains care about change.
- But if it doesn't change, then brains ignore it.
- Are parts of the world invisible to us that should be obvious?
- E.g. Our retina’s blind spot or maybe some constant cosmic rain that we’ve all ignored.
...
- Like plants working toward maximizing sunlight (phototropism), brains may work towards maximizing information (infotropism).
...
- Our brain regions only appear stable because they're locked in a cold war but everything is on a hair-trigger's edge.
...
- Evidence from the case of Alice, a child missing her right hemisphere, shows that territory is shared/squished.
...
- The Hubel and Wiesel experiments with alternating eye strip show
    - That brain maps aren't innate.
    - Maintaining territory is activity-dependent.
- The sensory homunculus looks that way because of the same competition. The areas that send the most information get the most brain area.
- For the system, at the small scale, the rule is competition over resources. At the large scale, we get emergent phenomenon such as the stretching, shrinking, and sharing of brain area.
...
- The threshold is around 8 years old.
...
- Stable data solidify first.
- E.g. Sound data is mostly consistent while body data changes as we go through puberty as we get stronger, weaker, fatter, thinner, or use new equipment.
...
- However, retaining the plasticity of a child isn't always a good thing. If plasticity didn't decline, we would remain as helpless as an infant.
- We would also lose our memories and this is horrific.
...
- One of the most important aspects of livewiring: the phenomenon of memory.
- When you learn a new fact, there are physical changes in the brain.
- Lashley's rat-treat experiment showed that memory isn't localized to any one spot but rather that it's distributed across the brain.
...
- Brains don't suffer from this problem which means that something more is happening.
- The first solution is to prevent the entire system from changing at once. This is done with neuromodulators.
- The brain changes only when it needs to, where it needs to.
...
- Review of patient H.M. and that the hippocampus is crucial to learning new facts but not critical for remembering facts that had already been learned.
...
- Plastic changes are distributed over a spectrum of time, not simply stored as all-or-none changes.
- A skill you thought was gone was still present, burned deep into the circuitry of the brain.
...
- Human brains don't stored memories the way computers do (but it is similar with the way caching works).
- In the pace layer model, slow layers provide a framework for the fast layers.
- New memories must fit into your current collection of memories.
...
- E.g. Birds with feathers fly but ostriches can't fly.
...
- Why are we still building hardwired machines that end up discarded?
...
- Instead of predefined circuitry, a better approach is to build a system that actively improves, self-adjusting to reach its goals.
...
- It puts our probes into the world by kicking things, touching, and knocking.
