...
- Since the brain is a physical design that performs certain functions, its design must obey general principles of engineering.
...
- We should care about small/primitive organisms because the mechanisms they evolved for these computations are retained in our own neurons.
- The increase in computational complexity is rewarded by increasing capacity to inhabit richer environments.
- Little beasts compute only what they must and thus they only pay for what they use. This also applies to us.
- Any feature of brain evolution that has been retained for so long must be effective.
...
- E.g. Getting into the fight/flight response before danger has occurred.
...
- However, we can’t remember everything due to the cost so we should only remember significant events whereas insignificant events should fade.
- Persistent dangers eventually become ingrained in our neural wiring.
- E.g. Fear of snakes.
...
- Passive signals spread is governed by a square root law of dendritic diameter which prevents neural wires from being any finer and smaller.
...
- Asking why teases out the principles of the device and seeks to discover the fundamental causes of the device’s functions.
...
- Another looming point is cost, a sturdier machine can always be built but it will cost more and could be less competitive.
...
- Analogous to how mechanical engineers have a variety of screws and materials to build devices, nature has a catalog of pats in the form of DNA sequences to modify and use to build organisms.
- E.g. Tuning the protein opsin to capture different wavelengths of light.
- The reasoning behind the design principle “if one design is simple and another complicated, choose the complicated” is that when one part does two jobs, it doesn’t do either well.
...
- Once gain, it’s reiterated that the cost of maintaining the membrane potential, the -65 mV, is very high in the brain, more than 60% of the brain’s total energy use.
...
- One organism, E. coli, has a memory of one second that’s stored using chemistry. While this might not seem impressive, given its lifestyle, the memory is just as long as it should be.
- A brain provides its organism with a much longer record of the past and thus, it also provides it a more extended future for it to exploit.
...
- E.g. Governments, brains, stakeholders, CPUs.
...
- Pattern recognition can be made even more efficient by having the sensors themselves be motorized to improve the search space.
...
- Information to C. elegans comes from three places: outside, inside, and the past.
...
- Diffusion is too slow so organisms turned to electrochemical signaling.
...
- This leads to another interesting idea, that life span and lifestyle are related to particular types of memory.
...
- This is also despite half a billion years of evolution, brain designs seem to have remained consistent, implying they’re neither random nor an accident.
- As organisms increase in size, they require more energy and thus need to increase their foraging volume.
- To forage effectively while avoid dangers, brains must commit resources to determining friends from foes, to discriminate edible from toxic.
...
- This requires a form of feedback and evaluations of reward prediction.
...
- There’s a low of diminishing returns for increasing information rate.
...
- The principle of minimizing wiring is seen by how the brain separates the wires that connect local circuits and with wires that connect distant circuits.
...
- Broad design rule for somatic motor neurons is that neurons that fire together should located together.
- Sensors differ greatly by cost. Olfaction is the cheapest but slowest, vision is in the middle, and audition is the fastest but most expensive.
- Auditory axons are thicker due to using higher firing rates.
...
- One exception is that olfactory sensors already signal at low rates and thus skip the thalamus. This may also explain why olfaction doesn’t pass through the thalamus.
...
- One might think that to capture the world’s infinite number of objects and categories, we would require infinite cortex area however, this isn’t applicable.
...
- E.g. A cricket’s load chirps risks desensitizing its own auditory system so to prevent this, the circuit that generates chirps also directly blocks inputs from the two ears for the duration of the chirp.
...
- E.g. If the probability distribution of four states is A (0.125), B (0.5), C (0.25), and D (0.125), then we can use a 3-bit code instead of a 2-bit code to be more efficient.
    - A 3-bit code is more efficient because we can use 1 bit to decide whether the state is either B or not-B, we can use 2 bits to decide between C or not-C, and we can use 3 bits to decide between A or D.
...
- This examples demonstrates one of Shannon’s discoveries, that it’s efficient to match a coding scheme to the statistical distribution of the states being coded.
...
- There are two types of redundancy
    - Less extreme form of repetition
        - Neural circuits commonly use lateral and self-inhibition to remove this type of redundancy to only send what’s needed.
    - Less information than they could because they’re used too frequently/rarely
        - Retinal neurons use this strategy
...
- During an AP, when the sodium channel opens for 1 ms and allows 6000 sodium ions into the cell, the sodium-potassium pumps use 2000 ATP molecules to pump those ions back.
...
- The one key task of neurons, to use allostery to send an electric signal somewhere far and fast.
...
- Synapse cleft width appears to optimally balance transmitter concentration at the postsynaptic receptors and electrical resistance. This also illustrates symmorphosis at the nano scale.
- For neurons to transmit at high frequencies, a fast rising chemical signal must also fall fast to enable the next signal.
...
- Vesicle release due to an AP is random and may release or it may not. While this uncertainty in neurotransmitter release introduces noise, thereby reducing information, it can also be an advantage.
...
- APs travel faster and avoid decay by regenerating as they go. This makes them travel farther and more resistant to noise.
...
- Another way to make computations more efficient is for performing computations locally.
...
- E.g. The command neuron that triggers the escape response of an invertebrate such as the squid giant axon at about 1 mm thick.
...
- However, if an axon were to be completely wrapped by a myelin sheath, the AP would eventually decay so there are certain gaps in the myelin sheath for the AP to regenerate.
...
- Another benefit of concentrating the sodium channels is to improve energy efficiency but this doesn’t explain why white matter uses three times less energy than grey matter.
...
- To capture the energy of a single photon, it is amplified chemically by a protein circuit in a highly structured physical context.
- There’s a close match between structure and function at this scale to increase efficiency.
- To deal with noise, rod cells integrate a graded signal but thresholds it to remove by amplifying larger signals.
![Figure 8.5](figure8-5.png)
- During daylight, rods shut themselves off and go into a sleep mode while cones take over.
- A cone costs somewhat more to operate than a rod, but it is also more sparsely distributed, making up only 5% of the retina.
...
- Flies that are slow have slow eyes while flies that are fast have faster eyes. The behavior is matched at the neuron-level to enable the behavior.
...
- Recall that higher spike rates need wider axons that use disproportionately more space and energy.
...
- Olfactory neurons collect information at low rates as they don’t track the location of odorant particles and they only track a single type of protein per sensor.
...
- However, cones collect too much information for it to be directly recoded into spikes and it is too much for a single synaptic stage like audition.
...
- The second stage reduces the event rate by throttling down to a sparse code.
...
- The eye must capture, process, and send an image under 100 ms.
...
- The retina’s architecture has been thoroughly mapped so we can start explaining the design.
...
- Why the asymmetrical response? Natural scenes tend to contain more dark contrasts so efficient design should allocate more resources to transmitting dark.
...
- The bipolar cells have two tasks
...
- Ganglion cells work together in an array to capture a scene. The spacing between cells matters because that defines the resolution of the scene.
...
- Distribute cells coding high frequencies sparser and cell coding low frequencies denser.
...
- Reasons why ganglion cell types are conserved across species
    ...
    - Once a design is optimal, it is preserved due to no pressure to change.
...
- This saving in space and energy carries forward in the cortex by allowing postsynaptic neurons to operate at lower rates.
- This appears to be the LGN’s main computational task and for other thalamic relays as well.
...
- A simple cell is mostly silent which is puzzling since many simple cells are required for a path of visual space. How much idle capacity should be tolerated?
...
- The rule is
    - When neurons are expensive and spikes are cheap, spike frequency to increase information per neuron.
    - When neurons are cheap and spikes are expensive, spike infrequency to increase information per spike.
...
- DN increases efficiency by
    ...
    - Ensures signaling range is used efficiently.
    - Reduces correlations between spikes.
    - Advances pattern recognition.
- DN discards absolute values and establishes relative values.
- In the retina, DN is used to discard light level to encode the invariant property, contrast.
...
- From there, it depends on what information the downstream targets need. Since the downstream targets are functionally diverse, each needs different information.
...
- V2 performs the next stages of pattern analysis for
    ...
    - Motion
    - Stereopsis
- V2 reorganizes the information from V1 by integrating common features together. However, it still separates fast to fast, slow to slow, and color to color.
...
- Beyond V2, the brain’s task is to find what matters most and to do it quickly.
- Why V2? It appears to complete the process of coding a whole scene efficiently as it is the last visual area where a lesion causes blindness.
...
- At the higher cortical areas, where the dedicate circuits live, much of the information from V2 is discarded for efficiency.
...
- One way axons save wiring is by branching at an acute angle rather than a right angle (Y vs T).
...
- So to conserve volume, dendrites should stay thin and by staying thin, they must stay short.
...
- When two neurons share inputs, they stay close together because if they’re separated, it’ll require more wiring to reach them.
- This explains why topographical maps are universal and why higher cortical areas collect from lower cortical areas.
...
- Within the brain, different regions use different amounts of energy with auditory structures using the most energy because their neurons need to process high temporal frequencies.
...
- This reduces the gradient across axonal membranes which means neurons are more sensitive.
- To prevent this, glial cells are used to rapidly restore the potassium gradient.
...
- E.g. A car doesn’t adapt to changing conditions. It could if the car on a rough road could thicken its tires, stiffen its springs, and change gears ratios.
- Yet for biology, adaptive responses are the essence. Think evolution.
- E.g. Where skin sustains rough wear, it thickens to callus. Where muscle sustain weight, it strengthens.
- It seems that wherever we look, current experience is continually being used to update circuits to improve future performance.
- These adjustments occur in all brains and in all systems.
- E.g. In C.elegans to humans, in the spinal cord and cerebellum.
...
- Classic example of enlarged hippocampus and London taxi drivers.
- White matter also expands with learning, forming new circuits to send out information.
...
- Nor is the hippocampus unique as other cortical areas expand with practice.
- E.g. The cortical motor areas expand with motor practice, the visual word form area expands with literacy, and areas critical to social functioning expand with the size of the social network.
...
- However, in humans, our body stops growing at the start of adulthood, which means our brain stops growing.
...
- What seems trivial now were not when they were learned.
...
- We shouldn’t seek a special organ for information storage, it’s stored as it should be, in every circuit.
...
- Certain brain areas should specialize in maintaining high levels of plasticity, while other areas should specialize in retaining what they’ve learned.
...
- Cellular design for efficient information storage
    ...
    - Storage occurs by increasing the synaptic weight.
    ...
    - However, to accommodate for more resources at the synapse, the neuron must also invest is a larger axon, more transportation, and more resources.
...
- The origin of some of the principles comes from electronics, chip design, mechanical engineering, or biology.
